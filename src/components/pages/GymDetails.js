...
const [phoneStatus, setPhoneStatus] = useState("");
  ...
  function handleChange(e) {
    ...
    setApiError(""); // clear backend error while typing
  }
 ...
 async function handleSubmit(e) {
  ...
  try {
    ...
    const res = await ownerRegister(ownerData);

    // ✅ THIS IS IMPORTANT (backend success check)
    if (res.success) {
    ...
    setApiError(
      err?.response?.data?.message ||
        "Email or phone already exists. Please try another."
    );
  } finally {
    setLoading(false);
  }
